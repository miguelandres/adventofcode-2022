...
class Directory(name:String, parent: Directory?, var contents: HashMap<String, FileSystemNode> = hashMapOf()) : FileSystemNode(name, parent) {
...
fun calculateSizes(cwd: Directory): Map<String, Pair<Directory, Int>> {
    ...
    val mapSubDirs = cwd.contents.map { it.value }.filterIsInstance<Directory>().map { dir -> calculateSizes(dir) }
        .fold( mapOf<String, Pair<Directory, Int>>()) {
                a, b ->  a.plus(b)
    }
